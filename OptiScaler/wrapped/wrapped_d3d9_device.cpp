...
// Phase 3 INTZ: build and bind an INTZ-backed depth surface to replace the
// auto-depth that WrappedIDirect3D9Ex::CreateDevice suppressed. Called once,
// right after the wrapper is constructed, when the substitution path took.
//
// Owns one ref to both the texture and its level-0 surface — both released
// in InvalidateTrackedResources. The device internally adds its own ref to
// the surface when SetDepthStencilSurface binds it.
void WrappedIDirect3DDevice9Ex::InitAutoDepthIntz(UINT width, UINT height)
{
    ...
    {
        LOG_ERROR("Auto-depth INTZ CreateTexture failed: hr=0x{:08X}",
                  static_cast<uint32_t>(hr));
        return;
    }
    ...
    {
        LOG_ERROR("Auto-depth INTZ GetSurfaceLevel failed: hr=0x{:08X}",
                  static_cast<uint32_t>(hr));
        tex->Release();
        return;
    }
    ...
    if (FAILED(hr))
    {
        LOG_ERROR("Auto-depth INTZ SetDepthStencilSurface failed: hr=0x{:08X}",
                  static_cast<uint32_t>(hr));
        surf->Release();
        tex->Release();
        return;
    ...
    // Pre-arm the one-shot flags so they don't fire for this synthetic setup,
    // but DO allow the readback path to log its first result.
